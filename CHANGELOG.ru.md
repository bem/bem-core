...
- Появился полифил (`jquery__event_type_pointers` как jQuery-плагин),
  позволяющий использовать универсальные события для десктопных и тач интерфейсов.
...
- Вместо события `leftclick` следует использовать `pointerclick` (предоставляемый полифилом `jquery__event_type_pointers`).
