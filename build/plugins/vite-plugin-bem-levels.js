...
function scanDirectory(dir, blockName, modules, levelDir) {
    ...
    for (const entry of entries) {
        ...
        existing.push({
            ...
            levelDir,
            isRedefinition: false, // will be computed later
        });
 ...
 * Returns { name, deps } or null.
 *
 * Handles:
 *   modules.define('name', ['dep1', 'dep2'], function(provide, dep1, dep2) { ... })
 *   modules.define('name', function(provide) { ... })
 *   modules.define('name', ['dep1'], function(provide, dep1, prev) { ... })
 */
...
function generateBarrel(name, entries, rootDir) {
    ...
    const exportName = safeIdentifier(name);
    lines.push(`import ${exportName} from '${basePath}';`);

    // Import redefinition files as side-effects
    for (let i = 1; i < entries.length; i++) {
        const redef = entries[i];
        const redefPath = './' + relative(rootDir, redef.filePath).replace(/\\/g, '/');
        lines.push(`import '${redefPath}';`);
    }
