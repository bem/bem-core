                    ...
                    History.checkerFunction = function(){
                        ...
                        if ( documentHash !== lastDocumentHash ) {
                            ...
                            if ( iframeHash !== documentHash ) {
                                //History.debug('4: hashchange.checker: iframe hash change', 'documentHash (new):', documentHash, 'iframeHash (old):', iframeHash);
                        ...
                        // The iFrame Hash has changed (back button caused)
                            ...
                            //History.debug('4: hashchange.checker: iframe hash out of sync', 'iframeHash (new):', iframeHash, 'documentHash (old):', documentHash);
        ...
        if ( History.emulated.pushState ) {
            ...
            History.onHashChange = function(event){
                ...
                if ( discardObject ) {
                    ...
                    if ( History.getHashByIndex(-2) === History.getHashByState(discardObject.forwardState) ) {
                        ...
                        //History.debug('4: History.onHashChange: go backwards');
                        History.back(false);
                    } else {
                        // We are going forwards
                        //History.debug('4: History.onHashChange: go forwards');
            ...
            History.pushState = function(data,title,url,queue){
                ...
                if ( queue !== false && History.busy() ) {
                    // Wait + Push to Queue
                    //History.debug('4: History.pushState: we must wait', arguments);
                    History.pushQueue({
                        scope: History,
            ...
            History.replaceState = function(data,title,url,queue){
                ...
                if ( queue !== false && History.busy() ) {
                    // Wait + Push to Queue
                    //History.debug('4: History.replaceState: we must wait', arguments);
                    History.pushQueue({
                        scope: History,
