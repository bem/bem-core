...
* `<meta>` с атрибутом `name` в значении `'viewport'`. Элемент позволяет управлять масштабированием страницы. По умолчанию, масштабирование отключено. Для включения добавьте в поле `head` BEMJSON-декларации элемент `meta` со специализированным полем `zoom` – `true`.
