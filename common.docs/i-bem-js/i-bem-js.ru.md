...
Использовать в [декларации](#decl) имя блока в качестве первого аргумента метода BEMDOM.decl.
...
```js
modules.define('button', ['i-bem'], function(provide, BEMDOM) {
...
Контейнером, которому делегируется обработка БЭМ-событий, может
...
`liveBindTo([elem], event, handler)`. В статических методах декларации блока зарезервированно свойство `live` для подписки на делегированные DOM-события.
...
на любой элемент `item` в меню. Не важно, существовал ли этот элемент в момент инициализации экземпляра.
...
Если в декларации блока задано свойство `live`,
...
Если инициализацию блока не нужно откладывать (требуется
[автоматическая инициализация](#init-auto)), следует вернуть `false` из функции, заданной значением свойства `live`.
