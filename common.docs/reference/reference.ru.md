...
1. tag: `<div>`
1. js: `false` (атрибут `data-bem` и класс `i-bem` DOM-узла отсутствуют)
1. bem:  `undefined` (к DOM-узлу блока `block-name` добавляется `class="block-name"`)
1. cls: `undefined`  (у DOM-узла блока `block-name` нет дополнительных css-классов)
1. mix: `undefined` (у блока нет микса)
1. jsAttr: `undefined` (у блока нет `data-bem` атрибута)
1. attrs: `undefined` (у DOM-узла блока нет атрибутов)
1. content: `content` блока.
