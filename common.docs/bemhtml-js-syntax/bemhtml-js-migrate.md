...
**NB** Чтобы произвольные подпредикаты, относящиеся к полям входного BEMJSON, корректно обрабатывались в dev-среде необходимо использовать конструкцию вида `match(function() { return this.ctx.foo })`. 
Это связано с тем, что в dev-среде поле `this.ctx` может не быть определено в момент выполнения шаблона.
...
**Шаблон 1.** Задает для блока `logo` тэг `img` и соответствующий набор атрибутов. 
...
**Шаблон 2.** Задает для блока `b-page` тэг `html` и запрещает генерацию класса из имени БЭМ-сущности. 
...
**Шаблон 3.** Для элементов блока `b-text` устанавливает тэг в соответствие с именем элемента, определенным во входном BEMJSON. Если во входных данных для элемента задано поле `id`, присваивает значение поля атрибуту `id`. 
...
block('b-text')(
    ...
    match(this.elem).tag()(this.ctx.elem)

    match(this.elem).match(function() { return this.ctx.id}).attrs()({ id: this.ctx.id  })

)
```

**Шаблон 4.** Для блока `b-bla` по умолчанию задает тэг `span`. Если во входных данных задан модификатор блока `o-mode` в значении `v2`, меняет тэг на `a`. Затем к блоку примешивается модификатор элемента `m2` в  значении `v2` и указывается, что блок содержит JavaScript.
...
**Шаблон 5.** Оборачивает блок `b-inner` в блок `b-wrapper`. Для этого в моде `default` фрагмент входного BEMJSON подменяется блоком `b-wrapper`, содержащим изначальный фрагмент входных данных.
...
**Шаблон 6.** Для элемента `e1` блока `b-bla` по умолчанию задает тэг `span`. Если во входных данных определено поле `url`, меняет тэг на `a` и задает содержимое поля в качестве значения атрибута `href`. При совпадении с нестандартной модой `reset` значения атрибута `href` устанавливается равным `undefined`.
