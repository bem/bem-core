...
При использовании фрагмента входного BEMJSON `this.ctx` с конструкцией `applyCtx` в dev-среде, может произойти зацикливание в ходе выпонения шаблона. Чтобы этого избежать можно использовать защиту от зацикливания в сочетании с конструкцией `applyNext`:

```js
block('b-inner').match(!this.ctx._done).def()(applyNext(function() { return this.ctx = {
			block:'b-wrapper',
			_done: true,
			content: this.ctx
		})})
