...
**FIXME** этот пример убрать
	...
	DOM.decl(/* селектор блока */,
		...
		{
			/* статические методы */
			live: function() { ... } // условия инициализации по событию
		}
	);
...
* `Boolean`.<br/> Если `live` установлено в значение `true`, экземпляр
  блока будет инициализирован только при обращении к нему. Подробнее
  см. раздел [Взаимодействие блоков](#ibc).

**FIXME** только при попытке получения экземпляра блока. См. 

**FIXME** дать пример здесь

* `Function`.<br/>
  Функция, указанная в качестве значения `live`, будет выполнена один
  раз — при попытке инициализации первого экземпляра блока заданного класса.
**FIXME** уточнить с классом.

**FIXME**	В этой функции можно выполнить подписку на события, при
  наступлении которых нужно инициализировать блок. Помимо этого можно
  подписываться на другие делегированные события.
  И возвращать из live false

**FIXME** второй кейс для live: описываем делегированные события, но
  сам блок при этом автоматически инициализируем, для этого вернем
  false. Дать второй пример на этот кейс. преимущества делегированных
  событий, но при этом блок может требовать инициализации сразу
  (нельзя отложить инициализацию блока).
  любая live-инициализация — это подписывание на делегированные
  события. но не любые делегированные события означают ленивую
  инциализацию. 
  

Ниже приведен пример блока `square`, экземпляры которого будут
инициализированы по первому клику на соответствующем DOM-элементе. При
инициализации будет вызван метод `_onClick` экземпляра блока:
	...
	DOM.decl('square',
		...
		},
		{
			...
			live: function() {
	            ...
	            this.liveInitOn(...) // ?? вроде и не особо нужен
				
				this.liveBindTo('click', function(e) { // а это во
					второй юзкейс
					this._onClick(e);
				});
			} 
...
В коде примера использован хелпер `liveBindTo`, который позволяет
привязать callback-функцию к DOM-событию на блоке. Функция будет
вызываться каждый раз при наступлении DOM-события на экземпляре блока.
Callback-функция принимает один параметр — **FIXME**. `jQuery.Event`

**FIXME** написать, что это именно делегированное событие


Полный список хелперов для подписки на DOM-события экземпляров блоков
...
**FIXME**: DOM-события — для внутренней реализации блоков, BEM-для
  внешнего взаимодейтсвия. сделать NB про то, что нельзя пользоваться

**FIXME** вся работа с DOM-событиями реализована с помощью jQuery.
