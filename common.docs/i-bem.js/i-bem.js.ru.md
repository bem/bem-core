...
Фреймворк i-bem.js позволяет разложить клиентский JavaScript на компоненты в терминах БЭМ:
 ...
 блока. Каждому экземпляру блока соответствует JS-объект, динамически
 создаваемый в памяти браузера и хранящий состояние данного
 экземпляра. JS-объект хранит ссылку на DOM-элемент, к которому
 привязан данный экземпляр блока.
 + **Элементы** — В DOM-дереве элементам блока соответствуют узлы,
   вложенные в DOM-узел блока. Элементы блока доступны через
   [JS-API](#elem-api) экземпляра блока.
 + **Модификаторы** — Хранят информацию о состоянии блока и его
   элементов. Модификаторы представляют собой особые классы на
   DOM-узлах блока и элементов. Управление модификаторами производится
   через [JS-API](#mods-api) экземпляра блока.
...
(привязанный к DOM-элементу), необходимо переопределить ymaps-модуль `i-bem__dom`. 

Блоки декларируются с помощью метода `DOM.decl`, принимающим три
параметра:
...
* Модификатор может не иметь значения. Например, `disabled`. При
  выставлении модификатора с неуказанным значением i-bem.js
  автоматически присваивает ему значение `true`.
...
привязанного к DOM-элементу), необходимо вызвать метод `DOM.create`,
который вернет экземпляр блока:
...
**FIXME** пример кода с поиском блока
...
Поле, содержащее параметры блока **FIXME** — `{Object}`
