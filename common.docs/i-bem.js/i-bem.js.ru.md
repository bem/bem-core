...
JS-объекты экземпляров блоков, [не привязанных к DOM-дереву](#i-blocks), хранятся в
структуре данных `BEM.blocks`. При необходимости доступа к таким
блокам следует использовать конструкцию:
