...
Фреймворк i-bem.js позволяет разложить клиентский JavaScript на компоненты в терминах БЭМ:
 ...
 элементов интерфейса. Например, все кнопки могут быть реализованы с
 использованием блока `button`. Тогда, в соответствии с
 БЭМ-методологией, `button.css` определяет внешний вид всех кнопок, а
 `button.js` — логику их работы.
 ...
 состояние данного экземпляра. JS-объект хранит ссылку на DOM-элемент,
...
**FIXME** поставить ссылки на разделы справочника i-bem, i-bem__dom

* Модуль [i-bem](https://github.com/bem/bem-core/blob/v1/common.blocks/i-bem/i-bem.vanilla.js).<br/>
  ...
  node.js).
* Модуль [i-bem__dom](https://github.com/bem/bem-core/blob/v1/common.blocks/i-bem/\_\_dom/i-bem\_\_dom.js).<br/> Базовая
 ...
 * JS-объект, привязанный к DOM-элементу, обрабатывает происходящие на
   нем события и хранит состояние данного экземпляра блока.
 * i-bem.js дает возможность реализовать JS-компонент (блок), не
   привязанный к DOM, но имеющий API, аналогичное обычным блокам.
...
**NB** на некоторых уровнях переопределения для передачи блока может
использоваться другой HTML-атрибут. Например, в библиотеке bem-bl для
блоков сенсорных интерфейсов используется атрибут `ondblclick`.
...
**NB**: Оформлять инфраструктурный код в виде блока без
  ...
  БЭМ-блоков (состояния, выражаемые модификаторами, события и
...
2. Описание блока — хэш.<br/>  Например, так может выглядеть декларация
   методов для блока `button` с модификатором `pseudo` в значении
   `yes` (описывает поведение псевдокнопок):
	...
	Другой пример: блок `checkbutton` наследует блокам `button` и `checkbox`:
...
блока переключаться между значениями `'green'` и `''` модификатора
`color`, если не выставлен модификатор `disabled`:

	DOM.decl('square', {
		onSquareClick: function(e) {
			if ( !this.hasMod('disabled') ) {
				this.toggleMod('clicked', '');
				}
		}
		}, {}
	);
...
4. удаление модификатора `modName` (эквивалентно установке в качестве
   значения модификатора пустой строки `''` или установке простого
...
инициализация одного из них (появление модификатора `block_js_inited`)
...
первой функцией, выполняющейся при создании блока. **FIXME** уже не
первой, учитывая beforeSetMod, уточнить? Такой триггер можно
...
момент наступления события `domReady`. Для этого необходимо
...
* `Boolean`.<br/>
  ...
  блока. Подробнее см. раздел [Взаимодействие блоков](#ibc).
...
* `Function`.<br/>
  ...
  [описанием в декларации блока](#decl-selector).
...
привязать callback-функцию к событию на блоке. Функция будет
вызываться каждый раз при наступлении события на экземпляре блока.
Callback-функция принимает один параметр — **FIXME**.

Полный список хелперов для подписки на события экземпляров блоков
...
* **Статический бинд** — подписка на событие всех блоков с заданным
  ...
  передаются дополнительные параметры:
	...
	* контекст, в пределах которого нужно слушать события;
	* имя события;
	* функция-обработчик;
...
поиск вложенного блока `b-link`, чтобы подписаться на его событие
...
* `{jQuery} domElem` — DOM-узел, на котором размещен блок.
...
* `unbindFromDomElem(domElem, event)` — Отменить подписку на событие
  заданного DOM-узла.
...
* `unbindFromDoс(event)` — Отменить подписку на событие
  заданного DOM-узла.
...
* `unbindFromWin(event)` — Отменить подписку на событие
  заданного DOM-узла.
