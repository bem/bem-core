...
понять его основные принципы, пройдя семь простых шагов.


## Шаг 1. Инициализировать проект

Используем заготовку проекта от разработчиков БЭМ:
...
**NB: Если в проекте не определены шаблоны блока, HTML генерируется по дефолтным шаблонам из библиотеки `bemhtml`**
...
Поместим имя в специфическое для нашего блока поле данных — `name` (исправим `test.bemjson.js`):

    { block: 'b-hello', name: 'BEMHTML' }

А для генерации шаблонного текста приветствия создадим BEMHTML-шаблон. 
...
**NB: Этапы генерации HTML, не переопределённые в пользовательских шаблонах, выполняются дефолтными шаблонами библиотеки BEMHTML.**
...
С развитием проекта может возникнуть требование усложнить блок `b-hello` — выводить 
сразу несколько приветствий для заданного списка имен. 

Наиболее простая модификация исходных данных в этом случае — заменить имя в поле `name` массивом строк в поле `names`.
...
## Шаг 7. Оформление и поведение блока (CSS и JS)
